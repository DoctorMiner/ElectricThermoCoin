...
contract ElectricThermoCoin is Ownable {
    IERC20 public token;
    ...
    /***
     * Get the latest Matic/USD price from the Aggregator
     */
    function getMaticToUsdPrice() public view returns (int256) {
    ...
    /***
     * Convert MATIC to USD based on the latest price from the Aggregator
     */
    function convertMaticToUsd(uint256 maticAmount) public view returns (uint256) {
        int256 maticToUsdPrice = getMaticToUsdPrice();

        uint256 usdAmount = (maticAmount * uint256(maticToUsdPrice)) /
    ...
    function buyToken(uint256 _amount) public payable {
        ...
        require(_amount >= 100000000000000, "Please enter minimum token!");
        ...
        uint256 _maticToUsd = convertMaticToUsd(1e18);
        uint256 _totalPayAmount = (( _amount * _price) / _maticToUsd) - 100 ;
